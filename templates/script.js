...
function setupEventListeners() {
    ...
    console.log("Event listeners configurati correttamente");
}
...
function renderTrendsTable(data) {
    ...
    // Crea i grafici dopo aver aggiunto le righe alla tabella - AUMENTATO TIMEOUT
    setTimeout(() => {
        try {
            ...
            data.forEach((item, index) => {
                try {
                    ...
                    console.error(`Errore nella creazione del grafico ${index}:`, e);
                }
            });
...
function createTrendChart(containerId, dataPoints) {
    ...
    try {
        ...
        ctx.innerHTML = `<span style="font-size:10px">1h: ${dataPoints[0].toFixed(1)} | 4h: ${dataPoints[1].toFixed(1)} | 7d: ${dataPoints[2].toFixed(1)}</span>`;
    }
}
