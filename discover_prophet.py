...
if __name__ == "__main__":
    ...
    df_final = pd.DataFrame() # Inizializza vuoto
    ...
    try:
        ...
        try: # Save CSV
            ...
            # Assicurati che discover_score_col sia nella lista se esiste
            if discover_score_col not in cols_csv and discover_score_col in df_final.columns:
                 cols_csv.insert(1, discover_score_col)
            # Filtra solo colonne esistenti
            cols_csv = [col for col in cols_csv if col in df_final.columns]
        ...
        if FETCH_RELATED_KEYWORDS and N_PROCESS_FOR_CONTEXT > 0:
            ...
            def fmt_suggs(row):
                ...
                # Gestisci caso in cui l'entità non è stata fetchata
                elif row.name < MAX_RELATED_TO_FETCH: return "<small>Error/None</small>" # Era target ma fallito
                else: return "<small>Not fetched</small>" # Non era target
            df_display['Keyword_Suggestions'] = df_display.apply(fmt_suggs, axis=1); print("    Keywords fetched.")
        else: df_display['Keyword_Suggestions'] = "<small>Disabled</small>" # Placeholder

        # Prepare df_html for rendering
        ...
        # Aggiungi colonne Avg Score rinominate e formattate
        for tf in CONTEXT_TIMEFRAMES:
             rn_col = f'Avg {tf.replace("now ","").replace("-"," ")}';
        ...
        # Seleziona solo le colonne finali e rinomina
        ...
        # Prepara contesto per Jinja
        ts_now = pd.Timestamp.now(tz='Europe/Rome').strftime('%Y-%m-%d %H:%M:%S %Z') # Timezone Italia
        context = {
            ...
            'script_version':'V8.3 (Corrected)' # Versione aggiornata
            }
        # Rendering e salvataggio
        try:
            env = Environment(loader=FileSystemLoader('.'), autoescape=True) # autoescape=True è più sicuro
