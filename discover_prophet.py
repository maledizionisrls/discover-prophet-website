...
TEMPLATE_FILENAME = "template.html" # Nome del file template HTML
CSS_FILENAME = "styles.css"
CSV_OUTPUT_FILENAME = "discover_prophet_data.csv"
CHECKPOINT_DIR = "checkpoint_data_v8_3"
...
if not proxy_base_string or "your_proxy_provider" in proxy_base_string or "username-country-{geo}" in proxy_base_string:
    raise ValueError("Proxy base string non configurata o non valida! Controlla lo script o la GitHub Secret 'PROXY_STRING'.")
...
proxies_to_remove_geo = {'aq', 'io', 'cf', 'bi', 'td', 'km', 'fk', 'cn', 'gw', 'va', 'ki', 'nr', 'nu', 'nf', 'kp', 'pg', 'sh', 'ws', 'tl', 'tk', 'tv', 'wf', 'eh', 'dj', 'gl', 'al', 'cu', 'ir', 'sy', 'kp'}
country_codes = sorted([code for code in original_country_codes if code not in proxies_to_remove_geo])
print(f"Proxy GEOs: Using {len(country_codes)} after removing {len(proxies_to_remove_geo)}.")
...
if not proxies_list_with_geo: raise ValueError("Proxy list is empty after filtering!")
print(f"Generated {len(proxies_list_with_geo)} proxies.")
...
class ConsistentBrowserProfile:
    ...
    def __init__(self, geo_code):
        geo_code=(geo_code or 'DEFAULT').upper(); self.geo_code=geo_code; self.locale_info=get_locale_for_geo(self.geo_code); poss_os=self.GEO_OS_PREFERENCE.get(self.geo_code, self.GEO_OS_PREFERENCE['DEFAULT']); os_g=random.choice(poss_os); self.os_platform=random.choice(self.OS_MAP.get(os_g, ['windows'])); self.os_name=os_g; self.browser='chrome'; self.user_agent=self.FALLBACK_USER_AGENT;
...
class AdvancedProxyManager:
    ...
    def get_proxy(self):
        with self.lock:
            ...
            if len(self.active_proxies) < self.max_concurrent and self.available_proxies:
                ...
                elif not proxy_url and proxy_str: warnings.warn(f"Invalid proxy URL format for {proxy_str}, no proxy used for this session.")
                ...
                else: print(f"!! CRITICAL: Failed to create session {geo}. Releasing."); del self.active_proxies[proxy_str]; self.cooldown_proxies[proxy_str] = time.time() + self.cooldown_seconds * 2; self.proxy_stats[proxy_str]["fail_session"] += 1; return None
            else: return None
    def get_pytrends_session(self, proxy_str, profile):
        ...
        try: custom_session = requests.Session(); custom_session.headers.update(profile.get_headers());
    ...
    def release_proxy(self, proxy_str, success=True, status_code=None, error_type=None):
        with self.lock:
            if proxy_str not in self.all_proxies: return
            ...
            if success: self.proxy_stats[proxy_str]["success"] += 1; self.proxy_consecutive_failures[proxy_str] = 0
                ...
                self.proxy_consecutive_failures[proxy_str] += 1; fails = self.proxy_consecutive_failures[proxy_str]; cd_mult = 1.2;
                ...
                else: self.proxy_stats[proxy_str]["fail_other"] += 1
                cd_end = time.time() + (base_cd * cd_mult)
                if fails >= PROXY_FAILURE_THRESHOLD: penalty_mult = 1 + (PROXY_FAILURE_COOLDOWN_MULTIPLIER * (fails - PROXY_FAILURE_THRESHOLD + 1)); add_cd = base_cd * penalty_mult; ct = time.time(); base_dur = max(0, cd_end - ct); final_dur = base_dur + add_cd; cd_end = ct + final_dur; print(f"    INFO: Proxy {geo} ({proxy_str[-10:]}) {fails} cons fails. CD ~{final_dur:.0f}s.")
    ...
    def get_proxy_stats_summary(self):
         with self.lock:
            ...
            for p, s in self.proxy_stats.items(): geo=self.all_proxies.get(p,'N/A'); f429=s.get("fail_429",0); fpte_s=s.get("fail_proxy_error",0)+s.get("fail_5xx",0)+s.get("fail_timeout",0); fops_s=s.get("fail_other",0)+s.get("fail_parse",0)+s.get("fail_session",0); tf=f429+fpte_s+fops_s;
...
try: proxy_manager = AdvancedProxyManager(proxies_list_with_geo, MAX_CONCURRENT_PROXIES, PROXY_USE_COOLDOWN)
except Exception as e: print(f"!!! CRITICAL ERROR initializing Proxy Manager: {e} !!!"); exit()
...
def extract_ordered_entities(max_retries=ENTITY_EXTRACTION_MAX_RETRIES, initial_wait=ENTITY_EXTRACTION_INITIAL_WAIT):
    ...
    while attempts < max_retries:
        attempts += 1; print(f"Attempt {attempts}/{max_retries}..."); proxy_info, status_code, error_type_str, release_success = None, None, None, False; proxy_str = None; geo_code = 'N/A' # Default geo_code
        try:
            ...
            for internal_attempt in range(max_internal_retries):
                ...
                except (requests.exceptions.ProxyError, requests.exceptions.ConnectionError) as e: print(f"      Internal attempt {internal_attempt+1}: Proxy/Conn Err. Retrying..."); status_code=None; time.sleep(random.uniform(3,5))
                except Exception as e: print(f"      Internal attempt {internal_attempt+1}: Request Err ({type(e).__name__}). Retrying..."); status_code=None; time.sleep(random.uniform(2,4))
            ...
            for script in scripts:
                if script.string and 'trends.HotTrendsWidget.render' in script.string:
                    ...
                    if match:
                        ...
                        if isinstance(data, list) and data and isinstance(data[0], list): entities = [item.get('title') for item in data[0] if isinstance(item, dict) and 'title' in item];
                        if entities: ordered_entities_found = entities; found_data = True; break
                        except Exception as e: print(f"      JSON/Parse error: {e}"); error_type_str = 'parse_fail'; # Continue search
...
def get_trends_scores(keywords, timeframe):
    attempts = 0; current_backoff_429 = INITIAL_BACKOFF_SECONDS_429; current_backoff_other = 3.0; start_time = time.time(); proxy_info=None; proxy_str=None; max_proxy_attempts=max(3,MAX_RETRIES_PYTRENDS_CONTEXT); proxies_tried=set(); kw_hash = hashlib.md5(",".join(sorted(keywords)).encode()).hexdigest()[:6]; geo_code = 'N/A'
    while attempts < MAX_RETRIES_PYTRENDS_CONTEXT:
        ...
        try:
            ...
            except Exception as e: print(f"      [CTX KW:{kw_hash} T{attempts}] Session error ({geo_code}): {e}"); error_type_str = 'pytrends_session_error'; raise
            delay = random.uniform(MIN_DELAY_BETWEEN_REQUESTS, MAX_DELAY_BETWEEN_REQUESTS); time.sleep(delay);
            shuffled = keywords[:]; random.shuffle(shuffled); pytrends.build_payload(shuffled, timeframe=timeframe, geo='IT', gprop=''); df = pytrends.interest_over_time(); status_code = 200;
        ...
        except requests.exceptions.HTTPError as e: status_code = e.response.status_code if hasattr(e,'response') else None; print(f"      [CTX KW:{kw_hash} T{attempts}] HTTP Err {status_code or 'N/A'} ({geo_code})");
        ...
        except Exception as e: msg = str(e); error_type_str = f'other_{type(e).__name__}'; is_429 = '429' in msg; print(f"!!    [CTX KW:{kw_hash} T{attempts}] UNEXPECTED Err ({geo_code}): {type(e).__name__}: {msg[:150]}");
        if error_type_str=='other_ValueError' and 'Pytrends session' in msg: error_type_str='pytrends_session_error'
...
def get_all_context_scores(entities_subset, timeframe):
    ...
    def get_trends_scores_thread_safe(keywords_group, tf):
        with thread_limiter:
            try: return get_trends_scores(keywords_group, tf)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        ...
        completed_count = 0; failed_groups_count = 0
        # --- CICLO CORRETTO ---
        for future in tqdm(concurrent.futures.as_completed(future_to_group), total=len(groups), desc=f"Context {timeframe}"):
            group = future_to_group[future]
            try:
                result = future.result(timeout=MAX_WAIT_SECONDS * 3) # Usa timeout maggiore
                results_list.append(result)
                if result and all(s == 0 for s in result.values()): failed_groups_count += 1
            except concurrent.futures.TimeoutError: print(f"\n!!! TIMEOUT result group: {group} !!!"); results_list.append({k: 0 for k in group}); failed_groups_count += 1
            except Exception as exc: print(f"\n!!! ERROR result group {group}: {exc} !!!"); results_list.append({k: 0 for k in group}); failed_groups_count += 1
            finally: completed_count += 1 # Incrementa sempre nel finally
        # --- FINE CICLO ---

    print(f"    Combining results for {timeframe}..."); [all_scores.update(rd) for rd in results_list];
    final_scores = {entity: all_scores.get(entity, 0) for entity in entity_list}
    print(f"--- CONTEXT collection '{timeframe}' completed. Failed Groups: {failed_groups_count} ---");
    time.sleep(random.uniform(2, 5)); return final_scores

def create_trend_chart(entity_data, timeframes_for_chart):
    scores = []; labels = [];
    for tf in timeframes_for_chart: col = f'Score_Avg_{tf}';
    if col in entity_data and pd.notna(entity_data[col]) and entity_data[col] > 0: scores.append(entity_data[col]); labels.append(f"Avg {tf.replace('now ','').replace('-',' ')}")
    ...
    except Exception as e: print(f"Plotly error {entity_data.get('Entita','N/A')}: {e}"); return "<small>Chart error</small>"

def get_related_keywords(entity_name, proxy_manager, delay_seconds):
    suggestions = {'top':[],'rising':[]}; proxy_info=None; proxy_str=None; status_code=None; error_type_str=None; release_success=False; geo_code = 'N/A'
    try:
        for _ in range(3): proxy_info = proxy_manager.get_proxy(); time.sleep(random.uniform(1,2) if proxy_info is None else 0); if proxy_info: break
...
if __name__ == "__main__":
    ...
    try:
        ...
        try: pd.DataFrame({'Rank':range(1,len(ordered_entities)+1), 'Entita':ordered_entities}).to_csv(os.path.join(CHECKPOINT_DIR,"entities_ordered.csv"), index=False, encoding='utf-8-sig'); print("Checkpoint saved: entities_ordered.csv")
        ...
        if FETCH_VOLUME_CONTEXT:
            ...
            for run in range(1, CONTEXT_N_RUNS + 1):
                ...
                for tf in CONTEXT_TIMEFRAMES:
                    scores_run = get_all_context_scores(entities_for_context, tf); # Usa la funzione corretta
                    for entity, score in scores_run.items(): timeframe_context_results[tf][entity].append(score)
                    try: pd.DataFrame({'Entita': list(scores_run.keys()), f'Score_{tf.replace(" ","_")}_Run{run}': list(scores_run.values())}).to_csv(os.path.join(CHECKPOINT_DIR, f"context_run{run}_{tf.replace(' ','_')}.csv"), index=False, encoding='utf-8-sig');
            ...
            for tf_agg in CONTEXT_TIMEFRAMES: avg_col=f'Score_Avg_{tf_agg}'; avg_map={e: sum(s)/len(s) if s else 0 for e, s in timeframe_context_results[tf_agg].items()}; df_final[avg_col] = df_final['Entita'].map(avg_map).fillna(0); print(f"      Avg score calculated for {tf_agg}.");
        ...
        if score_cols: num = 1 + df_final[score_cols].sum(axis=1); den = np.log1p(df_final['Rank']); df_final[discover_score_col] = (num / den).fillna(0); print(f"    '{discover_score_col}' calculated.")
        ...
        df_final = df_final.sort_values(by=discover_score_col, ascending=False).reset_index(drop=True); df_final.insert(0, 'Final_Rank', range(1, len(df_final) + 1));
        try: # Save CSV
            cols_csv = ['Final_Rank', discover_score_col, 'Rank'] + score_cols + ['Entita'];
            if discover_score_col not in cols_csv and discover_score_col in df_final.columns: cols_csv.insert(1, discover_score_col)
            cols_csv = [col for col in cols_csv if col in df_final.columns] # Filter existing cols
            df_final[cols_csv].to_csv(CSV_OUTPUT_FILENAME, index=False, encoding='utf-8-sig', float_format='%.3f');
        ...
        df_display['Trend_Chart'] = df_display.apply(lambda row: create_trend_chart(row, chart_tfs), axis=1); print(f"    {len(df_display)} charts generated.")
        if FETCH_RELATED_KEYWORDS and N_PROCESS_FOR_CONTEXT > 0:
            ...
            def fmt_suggs(row): entity=row['Entita'];
            if entity in all_suggs: sugg=all_suggs[entity]; html="";
            if sugg.get('top'): html += f"<div><small><strong>Top:</strong> {', '.join(sugg['top'])}</small></div>"
            if sugg.get('rising'): html += f"<div><small><strong>Rising:</strong> {', '.join(sugg['rising'])}</small></div>"
            return f'<div class="related-keywords">{html}</div>' if html else "<small>N/A</small>"
            elif row.name < MAX_RELATED_TO_FETCH: return "<small>Error/None</small>" else: return "<small>Not fetched</small>"
        ...
        context = {'page_title':'Discover Prophet','css_filename':CSS_FILENAME,'current_timestamp':ts_now,'main_heading':'Discover Prophet','sub_heading':'Analisi Google Hot Trends (TV - Italia)','top_n_display':TOP_N_FINAL_DISPLAY,'html_table':html_table,'current_year':pd.Timestamp.now().year,'script_version':'V8.3 (Corrected)'}
        try:
