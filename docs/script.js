    ...
    try {
        ...
        if (typeof runMetadata === 'object' && runMetadata !== null) {
            ...
            if (prophecyTextEl && prophecyToggleBtn) {
                 ...
                 if(prophecyTextEl) prophecyTextEl.textContent = "Errore visualizzazione profezia."; // Fallback se manca bottone
            }

            // RIGHE RIMOSSE PER STATISTICHE VECCHIE
...
function setupProphecyDisplay(fullText, textElementId, buttonElementId, desktopCharLimit, mobileCharLimit) {
    ...
    if (!paragraph || !button || !fullText) {
        ...
        if(paragraph) paragraph.textContent = fullText || "Profezia non disponibile.";
        if(button) button.style.display = 'none'; // Nasconde bottone se manca testo o paragrafo
        return;
    }
    ...
    // Determina limite caratteri in base alla larghezza finestra (soglia 768px come da CSS)
    ...
    if (cleanedText.length > charLimit) {
        ...
        if (cutOff <= charLimit / 2 || cutOff === -1) { // Se non trova spazi o troppo all'inizio, taglia al limite
            cutOff = charLimit;
        }
        const truncatedText = cleanedText.substring(0, cutOff) + '…'; // Usa ellissi invece di ...
        ...
        paragraph.dataset.state = 'truncated'; // Stato iniziale
        button.innerHTML = 'Mostra tutto <i class="fas fa-chevron-down"></i>';
        button.style.display = 'inline-block'; // Mostra il bottone
        button.classList.remove('expanded'); // Assicura stato icona iniziale

        // Gestore eventi (assicura che sia aggiunto solo una volta)
        button.removeEventListener('click', handleToggleProphecy);
        button.addEventListener('click', handleToggleProphecy);
        ...
        button.removeEventListener('click', handleToggleProphecy); // Rimuove listener se non serve
    }
}
...
function handleToggleProphecy(event) {
    ...
    const paragraph = document.getElementById('prophecyText');

    if (!paragraph || !paragraph.dataset.state) return;
    ...
    if (state === 'truncated') {
        ...
        button.classList.add('expanded'); // Per eventuale stile icona
    } else {
        paragraph.textContent = truncatedText;
        button.innerHTML = 'Mostra tutto <i class="fas fa-chevron-down"></i>';
        paragraph.dataset.state = 'truncated';
        button.classList.remove('expanded'); // Per eventuale stile icona
    }
}


// Configura gli ascoltatori degli eventi (invariato)
...
// Renderizza la tabella delle tendenze (invariato)
    ...
    requestAnimationFrame(() => {
        ...
        data.forEach((item, index) => {
            ...
            try {
                ...
                if(container) { createTrendChart(containerId, chartData); }
            } catch (chartError) {
                console.error(`Errore chart ${containerId}:`, chartError);
                const errorContainer = document.getElementById(containerId);
                if(errorContainer) { errorContainer.innerHTML = '<span class="chart-fallback" style="color:red;">Chart Err!</span>'; }
            }
        });
...
// Funzione isEntityHotOriginal (invariata)
...
// Filtra la tabella (corretto colspan)
...
// Ordina la tabella (logica di sort per saturazione corretta)
function sortTable() {
    ...
    rows.sort((a, b) => {
        ...
        switch (sortBy) {
            ...
            case 'saturation_score':
                 // Assegna -Infinity se il valore è '-1', altrimenti parsa il numero.
